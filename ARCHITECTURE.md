...
ultrastrong synonym becomes public merely to complete a source checkpoint. The one-sided concrete
coefficient-series comparison is implemented through the existing `WeakBilin` test-space API;
the converse identification and the ultrastrong comparison remain tracked under IQ-010. The concrete predual
itself is the norm closure of the finite vector-functional span, certified through the established
`Predual` class; it is not a new trace-class or topology-bearing foundation.
