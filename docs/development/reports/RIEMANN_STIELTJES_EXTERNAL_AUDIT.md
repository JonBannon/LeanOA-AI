...
The recommended implementation is a small Sak-AI-local one-dimensional directed index whose
prescribed-cut coordinate is a finite set (common refinement by union), augmented by explicit
left/right endpoint-growth and whatever mesh/tag admissibility the source translation establishes.
It should use Mathlib's generic filter lemmas directly.  It should not import PNT+, the ICERM
branch, teorth/analysis, or current Mathlib's archived Riemann--Stieltjes integral.  This keeps the
existing `Iio`/`Ico` atom convention and avoids pulling a fixed-box, right-closed, norm-topological
integral into the public API.  `Finset ℝ` atTop is a candidate prescribed-cut coordinate, not by
itself a source-certified definition of Sakai's abstract integral.
...
displayed abstract integral at an arbitrary `λ₀`.  It gives no directed-set definition, no order
on divisions, and no explicit cofinal-refinement argument for inserting `λ₀`.

Nor does an audited contemporary Lean source supply that exact semantics:

- PNT+ uses `comap mesh (nhds 0)`, not `atTop` of the refinement order;
- Mathlib/ICERM BoxIntegral uses gauge/mesh filters with full-partition constraints on a fixed box,
  not a refinement-order net with escaping endpoints; and
- teorth/analysis defines scalar Darboux upper/lower values by `sInf`/`sSup`, not by a partition
  net.

Consequently, `Finset ℝ` ordered by inclusion is presently a **candidate semantics for prescribed
cuts**, not a source-equivalent definition of the entire abstract integral.  A source-fidelity
proof still has to relate the eventual-integral predicate to Sakai's shrinking-mesh divisions and
escaping endpoints (or justify a different equivalent formulation).  In particular, finite-set
`atTop` alone must not be advertised as the abstract Radon--Stieltjes integral merely because it
makes cut insertion cofinal.
...
- `Finset` inclusion order and union for the directed prescribed-cut coordinate, without claiming
  that this coordinate alone defines Sakai's integral;
...
The target cofinality theorem should say, in Sak-AI's own richer admissible-division index, that for
every finite set of prescribed cuts there is an eventual stage containing those cuts, while both
endpoints remain free to move outward and the required mesh/tag conditions continue to improve.
Restriction of a known ultraweakly convergent net to that subsystem should then be a short
application of the directly reusable generic filter API.  Proving that this integral filter is a
