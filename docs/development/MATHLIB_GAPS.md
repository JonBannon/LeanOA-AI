...
`σ(B(H),B(H)_*)`. The general
`Strong.isClosed_ofStrong_preimage_iff_ofUltraweak_preimage` and
`Ultraweak.testWeakRestrictionL` expose the exact reusable closure and nested-test-space
consequences needed by the source proof.

The final source theorem assembly is now complete. Sak-AI takes
...
later Sakai representation theorem and remains intentionally unclaimed. Since neither pinned nor
audited current Mathlib has the square-summable operator topology, Sak-AI introduces
`SquareSummableConvergenceCLM`; its generic application map reuses the existing local `lp.mapCLM`.
The carrier maps continuously to Mathlib pointwise/SOT, while on $B(H)$ every defining seminorm is
the GNS seminorm of a positive diagonal coefficient-series functional. This proves only the
source-required intrinsic-strong-to-ultrastrong direction, not the later topology equality. The
Kaplansky engine now accepts an explicit test-weak-closure target, and
...
implemented. The concrete square-summable-vector ultrastrong comparison is now implemented as
two one-way continuous identities, and the final global closedness assembly is complete.

Current Mathlib has also acquired `lp.mapCLM`, overlapping the older declaration in
`LeanOA.Lp.lpSpace`. The ultrastrong carrier deliberately reuses the local declaration so it does
not duplicate Sak-AI's own API. Migrating the local Lp extension layer to the upstream file is a
separate compatibility cleanup because the adjacent local Hölder declarations also overlap under
different names.
