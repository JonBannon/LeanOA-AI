...
NEEDED`; the concrete predual and one-sided coefficient-series layers were `LOCAL BRIDGE NEEDED`
and are now implemented in Sak-AI, while the relative-closure layer remains `LOCAL BRIDGE NEEDED`. Current
...
ultrastrong comparison and generalize the existing Kaplansky density engine to relative closure.
...
one-sided topology comparison are now implemented. The next gap is ambient-relative Kaplansky
density.
