...
The Verso Blueprint package in `docs/` builds and generates a multi-page site with 102 active nodes
and 173 statement-dependency edges. This includes all 87 nodes and 141 edges from the generated
legacy LeanBlueprint graph plus fifteen new connected nodes through canonical truncated-affine
recovery in Sakai 1.11.3. The manifest has 483 entries and reports no
...
truncated-affine recovery to the existing CFC positive part in Sakai 1.11.3. Stable labels,
...
All checks passed on 2026-08-30. `vbp check` reports `ok: true`, zero errors, and 483 manifest/cache
...
2. Continue within Sakai 1.11.3 by scratch-testing the fixed-projection ultraweak decomposition for
   an arbitrary competing lower family under the source hypotheses. The next theorem should derive
   the upper-support identity and strictly-below positive lower bounds after inserting a chosen cut;
   no resolution, integral, or PVM structure becomes public before that support test succeeds.
