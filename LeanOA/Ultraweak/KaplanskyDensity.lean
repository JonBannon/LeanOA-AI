...
section GeneralRestrictedPairing
...
/-- The closure of a subset of a specified dual normed space for the weak topology generated by a
chosen predual test submodule, transported back to the original ambient carrier.  This is a set
closure operation, not a second topology on `M`. -/
def testWeakClosure (V : Submodule ℂ P) (S : Set M) : Set M :=
  (WeakBilin.linearEquiv ℂ (testPairing (M := M) V)).symm ⁻¹'
    closure ((WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹' S)

@[simp]
theorem mem_testWeakClosure_iff (V : Submodule ℂ P) (S : Set M) (x : M) :
    x ∈ testWeakClosure (M := M) V S ↔
      (WeakBilin.linearEquiv ℂ (testPairing (M := M) V)).symm x ∈
        closure ((WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹' S) :=
  Iff.rfl

/-- Pulling a transported test-weak closure back to the weak realization recovers the ordinary
topological closure there. -/
@[simp]
theorem preimage_testWeakClosure (V : Submodule ℂ P) (S : Set M) :
    (WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹'
        testWeakClosure (M := M) V S =
      closure ((WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹' S) := by
  ext x
  simp [testWeakClosure]

/-- Every set is contained in its transported test-weak closure. -/
theorem subset_testWeakClosure (V : Submodule ℂ P) (S : Set M) :
    S ⊆ testWeakClosure (M := M) V S := by
  intro x hx
  change (WeakBilin.linearEquiv ℂ (testPairing (M := M) V)).symm x ∈
    closure ((WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹' S)
  apply subset_closure
  simpa using hx

/-- Transported test-weak closure is monotone. -/
theorem testWeakClosure_mono (V : Submodule ℂ P) {S T : Set M} (hST : S ⊆ T) :
    testWeakClosure (M := M) V S ⊆ testWeakClosure (M := M) V T := by
  intro x hx
  change (WeakBilin.linearEquiv ℂ (testPairing (M := M) V)).symm x ∈
    closure ((WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹' S) at hx
  change (WeakBilin.linearEquiv ℂ (testPairing (M := M) V)).symm x ∈
    closure ((WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹' T)
  exact closure_mono (Set.preimage_mono hST) hx

/-- A transported test-weak closure is closed in the test-weak realization. -/
theorem isClosed_preimage_testWeakClosure (V : Submodule ℂ P) (S : Set M) :
    IsClosed ((WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹'
      testWeakClosure (M := M) V S) := by
  rw [preimage_testWeakClosure]
  exact isClosed_closure

/-- Minimality of transported test-weak closure. -/
theorem testWeakClosure_minimal (V : Submodule ℂ P) {S T : Set M}
    (hST : S ⊆ T)
    (hT : IsClosed
      ((WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹' T)) :
    testWeakClosure (M := M) V S ⊆ T := by
  intro x hx
  have hx' : (WeakBilin.linearEquiv ℂ (testPairing (M := M) V)).symm x ∈
      (WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹' T :=
    closure_minimal (Set.preimage_mono hST) hT hx
  simpa using hx'

/-- A test-weakly closed ambient set is fixed by transported test-weak closure. -/
theorem testWeakClosure_eq_self (V : Submodule ℂ P) {S : Set M}
    (hS : IsClosed
      ((WeakBilin.linearEquiv ℂ (testPairing (M := M) V)) ⁻¹' S)) :
    testWeakClosure (M := M) V S = S :=
  Set.Subset.antisymm (testWeakClosure_minimal V Set.Subset.rfl hS)
    (subset_testWeakClosure V S)

/-- Transported test-weak closure is idempotent. -/
@[simp]
theorem testWeakClosure_testWeakClosure (V : Submodule ℂ P) (S : Set M) :
    testWeakClosure (M := M) V (testWeakClosure (M := M) V S) =
      testWeakClosure (M := M) V S :=
  testWeakClosure_eq_self V (isClosed_preimage_testWeakClosure V S)
...
/-- Relative transform-membership form of Kaplansky density: the input need only belong to the
test-weak closure of the star subalgebra. -/
theorem kaplanskyTransform_mem_ultraweak_closure_of_mem_testWeakClosure
    {V : Submodule ℂ P}
    (hV : SakaiInvariantTestSpace (M := M) V)
    (S : NonUnitalStarSubalgebra ℂ M) [IsClosed (S : Set M)]
    {a : M} (ha : a ∈ testWeakClosure (M := M) V (S : Set M)) :
  ...
  rw [himage] at himageClosure
  have haSW : (show WeakBilin (testPairing (M := M) V) from a) ∈ closure SW := by
    simpa only [SW, mem_testWeakClosure_iff, WeakBilin.linearEquiv_symm_apply] using ha
  have haM : toMackey (mackeyTestPairing (M := M) V) a ∈ closure SM := by
    have ha_image : (show WeakBilin (testPairing (M := M) V) from a) ∈
        mackeyToWeakEquiv (M := M) (P := P) '' closure SM := by
      rw [himageClosure]
      exact haSW
    obtain ⟨y, hy, hya⟩ := ha_image
    have hya' : y = toMackey (mackeyTestPairing (M := M) V) a := by
      apply (mackeyToWeakEquiv (M := M) (P := P)).injective
      rw [hya]
      apply (WeakBilin.linearEquiv ℂ (testPairing (M := M) V)).injective
      change ofMackey (toMackey (mackeyTestPairing (M := M) V) a) = a
      exact ofMackey_toMackey _
    rwa [← hya']
...
theorem kaplanskyTransform_mem_ultraweak_closure {V : Submodule ℂ P}
    ...
    toUltraweak ℂ P (CStarAlgebra.kaplanskyTransform a) ∈
      closure (ofUltraweak ⁻¹' ((S : Set M) ∩ Metric.closedBall (0 : M) 1)) := by
  apply kaplanskyTransform_mem_ultraweak_closure_of_mem_testWeakClosure hV S
  apply (mem_testWeakClosure_iff V (S : Set M) a).2
  rw [hS.closure_eq]
  trivial
...
/-- If the ultraweak closure of a real-convex ambient set is a second ambient set, then the
Mackey closure for the whole specified predual is that same second set. -/
theorem mackey_closure_of_ultraweak_closure (C D : Set M)
    (hC : Convex ℝ C)
    (hUW : closure (ofUltraweak ⁻¹' C : Set σ(M, P)) =
      (ofUltraweak ⁻¹' D : Set σ(M, P))) :
    closure (ofMackey ⁻¹' C :
        Set (SakaiMackey (M := M) (⊤ : Submodule ℂ P))) =
      (ofMackey ⁻¹' D :
        Set (SakaiMackey (M := M) (⊤ : Submodule ℂ P))) := by
  let CM : Set (SakaiMackey (M := M) (⊤ : Submodule ℂ P)) := ofMackey ⁻¹' C
  let DM : Set (SakaiMackey (M := M) (⊤ : Submodule ℂ P)) := ofMackey ⁻¹' D
  let CW : Set (WeakBilin (testPairing (M := M) (⊤ : Submodule ℂ P))) :=
    (WeakBilin.linearEquiv ℂ (testPairing (M := M) (⊤ : Submodule ℂ P))) ⁻¹' C
  let DW : Set (WeakBilin (testPairing (M := M) (⊤ : Submodule ℂ P))) :=
    (WeakBilin.linearEquiv ℂ (testPairing (M := M) (⊤ : Submodule ℂ P))) ⁻¹' D
  let CU : Set σ(M, P) := ofUltraweak ⁻¹' C
  let DU : Set σ(M, P) := ofUltraweak ⁻¹' D
  ...
  have hMCimage : mackeyToWeakEquiv (M := M) (P := P) '' CM = CW := by
    ...
    · intro hx
      ...
      exact ofMackey_toMackey _
  have hMDimage : mackeyToWeakEquiv (M := M) (P := P) '' DM = DW := by
    ext x
    constructor
    · rintro ⟨y, hy, rfl⟩
      exact hy
  ...
  have hWU_D : topWeakUltraweakCLE (M := M) (P := P) '' DW = DU := by
    ext x
    constructor
    · rintro ⟨y, hy, rfl⟩
      exact hy
...
theorem mackey_closure_unitBall_of_ultraweak_closure (C₀ : Set M)
    ...
    closure (ofMackey ⁻¹' C₀ :
      ...
      (ofMackey ⁻¹' Metric.closedBall (0 : M) 1 :
        Set (SakaiMackey (M := M) (⊤ : Submodule ℂ P))) :=
  mackey_closure_of_ultraweak_closure C₀ (Metric.closedBall (0 : M) 1) hC₀ hUW
