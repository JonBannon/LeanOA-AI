...
omit [LocallyCompactSpace K] [R1Space K] in
/-- The continuous real-valued functions supported in `u` and taking values in `[0, 1]` form a
directed set under the pointwise order. -/
lemma ContinuousMap.directedOn_setOf_support_subset_Icc (u : Set K) :
    DirectedOn (· ≤ ·) {f : C(K, ℝ) | support f ⊆ u ∧ ∀ x, f x ∈ Icc 0 1} := by
  rintro f hf g hg
  refine ⟨f ⊔ g, ⟨?_, by simp; grind⟩, by simp⟩
...
theorem ExtremallyDisconnected.ofConditionallyCompletePartialOrderSupContinuousMap
  ...
  suffices h : ∀ s : Set C(K, ℝ), DirectedOn (· ≤ ·) s → s.Nonempty → BddAbove s → ∃ g, IsLUB s g by
    ...
    obtain ⟨f, hf⟩ :  ∃ f, IsLUB s f := by
      exact h s (ContinuousMap.directedOn_setOf_support_subset_Icc u)
        ⟨0, hs_zero⟩ ⟨1, hs_one⟩
